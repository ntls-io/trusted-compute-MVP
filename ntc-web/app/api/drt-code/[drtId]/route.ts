// app/api/drt-code/[drtId]/route.ts
...
/* -------------------- helpers -------------------- */
type RouteCtx = { params: { drtId: string } };

/* ---------------------  PUT  --------------------- */
export async function PUT(req: Request, ctx: RouteCtx) {
  
  const { drtId } = await ctx.params;

  const { userId } = await auth();
  if (!userId) return NextResponse.json({ error: 'Unauthorised' }, { status: 401 });

  const drt = await prisma.digitalRightToken.findUnique({ where: { id: drtId } });
  if (!drt)     return NextResponse.json({ error: 'Not found'    }, { status: 404 });
