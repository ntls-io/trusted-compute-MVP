...
#define GET_REQUEST "GET /health HTTP/1.1\r\nHost: 127.0.0.1:8080\r\n\r\n"
...
int main(int argc, char** argv) {
    ...
    // Add these lines to configure TLS 1.3
    ...
    * The actual certificate verification happens in my_verify_callback. */
    ...
    do {
        ...
        // If we got data and then got a close notification, that's OK
        if (ret == MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY && got_data) {
            mbedtls_printf("\nConnection closed by server after receiving data\n");
            exit_code = MBEDTLS_EXIT_SUCCESS;
            break;
        }

        // Only treat as error if we never got data
        if (ret < 0 && !got_data) {
